...
class HelpdeskTicketMerge(models.TransientModel):
    ...
    _description = 'Merge selected helpdesk tickets.'
    ...
    def merge_tickets(self):
        ...
        values = {'tag_ids': [(4, tag_id) for tag_id in tag_ids], 'user_id': self.user_id.id}
        if self.create_new_ticket:
            ...
            values['description'] = '\n'.join((self.target_ticket_id.description or '',
                                               self._merge_description(self.ticket_ids - self.target_ticket_id)))
        ...
        self.target_ticket_id.message_post_with_view(
            self.env.ref('spc.mail_template_ticket_merge'),
    ...
    def _merge_description(self, tickets):
        return '\n'.join(tickets.mapped(lambda ticket: _("Description from ticket %s: %s") % (
            ticket.name, ticket.description or _('No description'))))
    ...
    @api.model
    def default_get(self, ticket_fields):
        result = super().default_get(ticket_fields)
