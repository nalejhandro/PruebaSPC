...
class ProjectTaskMergeWizard(models.TransientModel):
    ...
    _description = 'Merge selected project tasks.'
    ...
    def merge_tasks(self):
        ...
        self.target_task_id.message_post_with_view(
            self.env.ref('spc.mail_template_task_merge'),
    ...
    def merge_description(self):
        return '<br/>'.join(self.task_ids.mapped(lambda task: "Description from task <b>%s</b>:<br/>%s" % (
    ...
    @api.model
    def default_get(self, task_fields):
        result = super().default_get(task_fields)
