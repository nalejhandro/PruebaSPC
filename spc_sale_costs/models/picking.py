...
class StockPicking(models.Model):
        ...
        if 'picking_type_id' in vals and 'group_id' in vals and \
            ...
            if picking_type.cost_on_sale_order:
                sale_order = self.env['sale.order'].search(
                    ...
                    limit=1)
                vals['cost_sale_order_id'] = sale_order and \
                    sale_order.id or False
            else:
    ...
    def write(self, vals):
        for picking in self:
            picking_type_id = 'picking_type_id' in vals and \
                vals['picking_type_id'] or picking.picking_type_id.id
            group_id = 'group_id' in vals and \
                vals['group_id'] or picking.group_id.id
            company_id = 'company_id' in vals and \
                vals['company_id'] or picking.company_id.id
            picking_type = self.env['stock.picking.type'].search(
                [('id', '=', picking_type_id)])
            if picking_type.cost_on_sale_order:
                sale_order = self.env['sale.order'].search(
